...
function 안녕(파라미터) {
    ...
    return 100 //자신을 불러온 자리(안녕()자리)에 100을 넣어라.
}

let 아규먼트 = 1000;
안녕(아규먼트) //변수 말고 1000이렇게 값을 직접 넣을 수 있음
console.log(안녕(아규먼트)+안녕(아규먼트)) //1000 hello world 1000 hello world 200 undefined가 출력됨.
...
//리턴값이 hello가 아님. hello는 실행되는 구문.
console.log(String(console.log('hello')) + String(console.log('hello'))) //undifinedundefined
*/
...
function 안녕() {
    console.log('hello')
    //리턴 구문 생략. undefiend가 반환됨
} 
안녕()
console.log(안녕() + 안녕()) //NaN이 나옴.
...
//1. 재사용성
//(재사용하지 않으 코드는 함수로 정의하나요? -> 가능하면 함수로 만드는 것을 권합니다.)
//2. 유지보수
// -> 3번 사항 + 변수의 스코프 제한
//3. 구조 파악이 용이하다.
// 참고만
...
함수1(10, 20, 30); //60이 나옴. 콘솔창에 console.log로 찍지 않았는데도 어떻게 60이 출력될까? 참고. 이것은 콘솔창의 기능임. 마지막 라인에 한하여 console.log()를 찍지 않아도 return값을 console창에 출력해줌.
함수1(10, 20, 50); //위에있는 10, 20, 30의 결과값은 나오지 않음. (마지막 라인에 한하여 해당되므로.)

//다음 실행 값은? 60 80 undefined가 나옴.사실 60 undefiend 80 undeifeind임. 60 다음의 undefined를 생략해준것.
console.log(함수1(10, 20, 30))
console.log(함수1(10, 20, 50))

// 필요 이상의 아규먼트를 넣었을 때
함수1(10, 20, 30, 40) // 40을 삭제하고 10 20 30만 계산됨. Error를 뿜지 않음. 60이 나옴.

// 필요 이하의 아규먼트를 넣었을 때
함수1(10, 20) //3개를 받아야 하는데 하나의 값이 없음. NaN이 나옴. c에 undefined가 들어갔기 때문. 10 + 20 +Undefined
/*
    function 함수1 (a, b, c) {
        return a + b + String(C)
    }
...
console.log(함수3(c=1000)) //1050 (1030이 정상). 값이 순서대로 들어감 a에 1000이 들어간것.
console.log(함수3(c=1000, a=2000)) //3030. (3020이 나와야 정상) //값이 순서대로 들어감. a에 1000 b에 2000이 들어간것.
...
함수4('Gold', true, true, true, '대화방 전체 백업 가능', true) //이렇게 호출하게되면 true가 뭔지 알 수없음. true가 어떤 값인지 확인하기 위해서 함수4가 선언되어 있는 파일로 가서 확인해야 함. 가독성이 떨어진다.

//RORO 기법. (함수에 아규먼트로 값을 전달할 때 오브젝트로 전달하게 되면 조금더 가독성이 높아지는 코드를 작성할 수 있다.)
function 함수4({ //회원등급권한
...
//////////초기값 설정1//////////
function 함수4({
    회원등급 = 'Gold', //(참고)중괄호나 소괄호 안에서는 개행이 가능함. 회원등급 : 'Gold'는 에러 발생
    글쓰기 = true,
    글읽기 = true,
    채널관리 = true,
    백업 = '부분가능',
...
함수() //(x)실행 안됨. 오브젝트가 들어가야 함. 마치 let {a=30, b=20, c=30} = undefined랑 같은것. 사실 undefiend도 없이 보낸것. 구조분해 할당은 일단 오른쪽에 {}가 있어야함.
...
// let {one = 1, two = 1} = {one: 100} 매핑이 되려면 오른족에 one 100이 있어야 one에 100이 들어가는데 {one:1 two:2} = {}는 매핌되는 값이 없는것. 그래서 매핑이 안됐기 때문에 그냥 {one:1 two:2가 그대로 남는 것}

// let {a=2, b=1, c=3} = {}
// let {a=2, b=1, c=3} = {a: 100} 구조분해할당으로 a가 100이됨. 그러나 b나 c는 없기 때문에 b=1 c=3이 남아있고 a만 변경되는 것. 이 성질을 이용!
// let {a=2, b=1, c=3} = {}의 의미는 오른쪽에 값이 없기 때문에 할당이 안되어서 오브젝트 값이 그대로 할당되게 해준다는 것. 그러면 a, b, c가 그대로 2, 1, 3으로 남아있는 것. 
...
// 재귀함수(top-down)는 위에서부터 아래로 내려가는 방식에서 많이 사용. 또한, 분할정복 코드에서 많이사용 (큰 문제를 쪼개서 합치면 문제를 해결하는 락드릴에서 재귀함수를 사용하면 쉽게 해결가능. 사실 분할정복도 같은 맥락)
...
a() // a 실행이 출력됨.
b() //실행 불가. b는 접근이 불가능한 함수. 안에서 선언된 변수나 함수는 밖에서 꺼내서 쓸 수 없다. 안에 있는 함수를 꺼내서 쓰는 것이 closer
