...
for (i in value2) { //키가 출력됨. 객체는 for of로 순회를 돌 수가 없음.
    console.log(i)
}

for (i of value2) { //불가능. of(배열의 값을 출력)로 순회를 돌아서 key와 value를 한꺼번에 출력하고 싶음. 
    console.log(i)
}

Object.entries(value) //entries는 [key, value] 쌍으로 이루어진 배열을 반환 [Array(2), Array(2), Array(2), Array(2), Array(2), Array(2)]
Object.entries(value2) // [Array(2), Array(2), Array(2), Array(2)]
...
for([i, j, ...k] of [[10, 20, 300, 1, 2], [30, 40, 500, 1]]) {
    console.log(i, j, k) //몇 개 들어올지 모르는 경우 스프레드 용법 사용(이런 경우는 많이 없음. 거의 쓰지 않음) i와 j에 들어가지 못하는 값들 [300, 1, 2]은 k에 전개되어서 들어감.
} 
...
[one1, two1, three1, ...four1] = '010-5044-2033-0000-01-02-03'.split('-') //000 01 02 03을 잃어버리면 안되니까  four1을 통해 전개해서 받아줌.
...
let {name, age} = data //name에 'houn' age에 '10' 이 들어감.

let {name, age} = {name: 'hojun', age: 10 } //이렇게도 가능. 값은 이름이 있을 때 값을 뽑아서 넣어줌.
